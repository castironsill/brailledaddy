...
IMPL.lou = {
	...
	translateString: function(table, inbuf, backtranslate) {
		...
		// BrailleDaddy patch (v1.2.0): the upstream easy-api passed the input
		// length and output-buffer capacity to liblouis in *bytes*, while
		// liblouis counts *widechars*. It also sized the output buffer equal to
		// the input. Braille output is frequently LONGER than the input (capital
		// indicators, number signs, etc.), so expanding translations overran the
		// allocation and corrupted the emscripten heap (intermittent abort() on
		// _free). We now pass the correct widechar counts and allocate a generous
		// output buffer. See vendor/liblouis/PATCHES.md.
		...
		var mode = 0;
		var charSize = this.charSize();              // bytes per widechar (2 or 4)
		var inlen = inbuf.length;                    // widechars in
		var outCap = inlen * 8 + 256;                // widechar capacity out (generous)

		var inbuff_ptr = this.mem.transfer(this.capi, inbuf);
		var outbuff_ptr = this.capi._malloc((outCap + 1) * charSize);

		var outlen_ptr = this.capi._malloc(4);
		var inlen_ptr = this.capi._malloc(4);

		this.capi.setValue(inlen_ptr, inlen, "i32");
		this.capi.setValue(outlen_ptr, outCap, "i32");
		...
		var success = this.capi.ccall(backtranslate ?
				...
				inlen_ptr, outbuff_ptr, outlen_ptr, null,
				null, mode]);

		if(!success) {
			this.capi._free(outbuff_ptr);
			this.capi._free(inbuff_ptr);
			this.capi._free(outlen_ptr);
			this.capi._free(inlen_ptr);
			return null;
		}

		var outbuff = this.mem.read(this.capi, outbuff_ptr, outlen_ptr);
